        ...
        if cond.count == 0:
            if cond.min_signatures != 0:
                raise NotImplementedError(
                    "Support for minimum number of different matching signatures is not implemented!")

            return "not (%s)" % _to_yara_condition(cond.condition)

        min_signatures = ""
        if cond.min_signatures != 0:
            min_signatures = " and %d of (%s)" % (
                cond.min_signatures, ",".join("$%s" % x for x in _get_subsignatures(cond.condition)))

        subpattern_count = " + ".join("#%s" % x for x in _get_subsignatures(cond.condition))

        return "(%s) and (%s) == %d%s" % (
            _to_yara_condition(cond.condition), subpattern_count, cond.count, min_signatures)
    elif isinstance(cond, CondMatchLess):
        min_signatures = ""
        if cond.min_signatures != 0:
            min_signatures = " and %d of (%s)" % (
...
rule %s {
    ...
    condition:
        (%s) and (%s) and (%s)
}""" % (name, "\n        ".join("$%s = { %s }" % ptrn for ptrn in patterns), target_type_condition,
        " and ".join("(%s)" % x for x in offset_conditions), _to_yara_condition(signature.condition))
